...
def constructBayesNet(gameState: hunters.GameState):
    ...
    manDistances0 = manDistances(variableDomainsDict[PAC], variableDomainsDict[GHOST0])
    manDistances1 = manDistances(variableDomainsDict[PAC], variableDomainsDict[GHOST1])

    # Adding noise to manhattan distances
    variableDomainsDict[OBS0] = addNoisyDistances(manDistances0, MAX_NOISE)
    variableDomainsDict[OBS1] = addNoisyDistances(manDistances1, MAX_NOISE)

    net = bn.constructEmptyBayesNet(variables, edges, variableDomainsDict)
    return net


# A function to calculate manhattan distances between pacman and ghosts
def manDistances(pacmanPositions, ghostPositions):
    res = []
    for pacmanPos in pacmanPositions:
        for ghostPos in ghostPositions:
            manDistances = manhattanDistance(pacmanPos, ghostPos)
            if manDistances not in res:
                res.append(manDistances)
    return res

# A function to add noise to manhattan distances
def addNoisyDistances(distances, maxNoise):
    res = []
    for distance in distances:
        for noise in range(-maxNoise, maxNoise + 1):
            noisyDistance = abs(distance + noise)
            if noisyDistance not in res:
                res.append(noisyDistance)
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        # Create a bayes net with all the factors that contain the evidence variables
        factors = bayesNet.getAllCPTsWithEvidence(evidenceDict)

        # Join all factors by variable
        for factorsToEliminate in eliminationOrder:
            factors, joinedFactor = joinFactorsByVariable(factors, factorsToEliminate)
            unconditionalVariables = joinedFactor.unconditionedVariables()

            # If the joined factor has only one unconditioned variable, discard the factor
            if len(unconditionalVariables) > 1:
                factor = eliminate(joinedFactor, factorsToEliminate)
                factors.append(factor)

        # Join and normalize all factors
        joinedFactors = joinFactors(factors)
        normalizedFactors = normalize(joinedFactors)
